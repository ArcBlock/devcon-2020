...
const { getHackthonBadge } = require('./hackathon/badge');
...
const ensureAsset = async (
  ...
  } else if (vcType.indexOf('DevCon2020HackathonBadge-normal') > -1) {
    display = createZippedSvgDisplay(getHackthonBadge(userDid, 0, data.name, sessionID));
  } else if (vcType.indexOf('DevCon2020HackathonBadge-team') > -1) {
    display = createZippedSvgDisplay(getHackthonBadge(userDid, 1, data.name, ''));
  } else if (vcType.indexOf('DevCon2020HackathonBadge-project') > -1) {
    display = createZippedSvgDisplay(getHackthonBadge(userDid, 2, data.name, ''));
  } else if (vcType.indexOf('DevCon2020HackathonBadge-winner') > -1) {
    display = createZippedSvgDisplay(getHackthonBadge(userDid, 3, data.name, ''));
  } else if (vcType.indexOf('DevCon2020HackathonBadge-popular') > -1) {
    display = createZippedSvgDisplay(getHackthonBadge(userDid, 4, data.name, ''));
  } else {
    display = createZippedSvgDisplay(
