  ...
  try {
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      for (let j = 0; j < projectMembers.length; j++) {
        const accountState = await ForgeSDK.getAccountState({ address: projectMembers[j].did });
        if (accountState.code === 'OK' && accountState.state) {
          const badgeType = 'DevCon2020HackathonBadge-project';
          // check if the account has got the nft
          // check mongodb
          const exist = await BadgeRecord.findOne({
            did: projectMembers[j].did,
            badgeType,
          });
          if (exist) {
            console.log(
              `The ${projectMembers[j].did} has got the DevCon2020HackathonBadge-project badge before, so just skip`
            );
            continue;
          }
          const asset = await ensureAsset(assetFactory, {
            ...
            type: 'badge',
            vcType: badgeType,
            name: `${project.name} - 项目徽章`,
            description: `Grant a DevCon2020 Hackathon Badge to project: ${project.name}`,
            location: 'Online',
            backgroundUrl: '',
