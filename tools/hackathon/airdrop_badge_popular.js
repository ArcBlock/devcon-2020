...
const populars = require('./popular_data');
...
(async () => {
  try {
    ...
    for (let i = 0; i < populars.length; i++) {
      ...
      const popularMembers = popular.members;
      for (let j = 0; j < popularMembers.length; j++) {
        const accountState = await ForgeSDK.getAccountState({ address: popularMembers[j].did });
        if (accountState.code === 'OK' && accountState.state) {
          const badgeType = 'DevCon2020HackathonBadge-popular';
          // check if the account has got the nft
          // check mongodb
          const exist = await BadgeRecord.findOne({
            did: popularMembers[j].did,
            badgeType,
          });
          if (exist) {
            console.log(
              `The ${popularMembers[j].did} has got the DevCon2020HackathonBadge-popular badge before, so just skip`
            );
            continue;
          }
          const asset = await ensureAsset(assetFactory, {
            ...
            type: 'badge',
            vcType: badgeType,
            name: `${popular.name} - 人气徽章`,
            description: `Grant a DevCon2020 Hackathon Badge to popular project: ${popular.name}`,
            location: 'Online',
            backgroundUrl: '',
