...
const winners = require('./winner_data');
...
(async () => {
  try {
    ...
    for (let i = 0; i < winners.length; i++) {
      ...
      for (let j = 0; j < winnerMembers.length; j++) {
        const accountState = await ForgeSDK.getAccountState({ address: winnerMembers[j].did });
        if (accountState.code === 'OK' && accountState.state) {
          const badgeType = 'DevCon2020HackathonBadge-winner';
          // check if the account has got the nft
          // check mongodb
          const exist = await BadgeRecord.findOne({
            did: winnerMembers[j].did,
            badgeType,
          });
          if (exist) {
            console.log(
              `The ${winnerMembers[j].did} has got the DevCon2020HackathonBadge-winner badge before, so just skip`
            );
            continue;
          }
          const asset = await ensureAsset(assetFactory, {
            ...
            type: 'badge',
            vcType: badgeType,
            name: `${winner.name} - 获奖徽章`,
            description: `Grant a DevCon2020 Hackathon Badge to winner: ${winner.name}`,
            location: 'Online',
            backgroundUrl: '',
